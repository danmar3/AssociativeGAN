...
from .encoder import Estimator, CallWrapper, LinearClassifier
from .gmm import GMM
...
class ExGan(tdl.core.TdlModel):
    ...
    def encoder(self, units, kernels=3, strides=None, pooling=2, stages=3,
                layer_type='plain', batchnorm='batchnorm'):
        tdl.core.assert_initialized(
            self, 'encoder', ['use_bias', 'equalized', 'embedding_size'])
        encoder = res_conv2d.ResNet(
            input_layer={'units': units[0]},
            hidden={
               'units': units,
               'kernels': kernels,
               'pooling': pooling,
    ...
    def classifier(self, n_classes):
        tdl.core.assert_initialized(self, 'classifier', ['encoder'])
        classifier = LinearClassifier(n_classes=n_classes,
    ...
    def classifier_trainer(self, xreal, yreal, regularizer=None,
        ...
        optim = estimator.get_optimizer(
            xreal, yreal, progress={'reset_multiplier': 3.0},
            **optimizer)
        return SimpleNamespace(
            estimator=estimator, optim=optim,
            variables=tdl.core.get_variables(self.classifier) +
            optim.optimizer.variables())

    def generator_trainer(self, xreal, batch_size, sim_alpha, learning_rate, beta1=0.0):
        ...
        loss = loss + sim_alpha * similarity
    ...
    def discriminator_trainer(self, xreal, learning_rate, beta1=0.0):
        tdl.core.assert_initialized(
            self, 'discriminator_trainer',
            ['generator', 'discriminator', 'discriminator_loss', 'pyramid'])
        train_step = tf.Variable(0, dtype=tf.int32, name='train_step')
        optimizer = tf.compat.v1.train.AdamOptimizer(
            learning_rate, beta1=beta1)

        xreal = [tf.stop_gradient(xi) for xi in self.pyramid(xreal)]
        zreal = tf.stop_gradient(self.encoder(xreal[-1]))
        xsim = self.generator(zreal, output="pyramid")
